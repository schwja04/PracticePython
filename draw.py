...
class TextCommand:
    def __init__(self,intertext, fontsize):
        self.intertext = intertext
        self.fontsize = fontsize

    def draw(self, turtle):
        turtle.write(self.intertext, font=('Arial', int(self.fontsize)))

    def __str__(self):
        return '<Command intertext="' + str(self.intertext) + '" fontsize="' + \
               str(self.fontsize) + '">Text</Command>'
...
class DrawingApplication(tkinter.Frame):
    ...
    def buildWindow(self):
        ...
        def parse(filename):
            ...
            for commandElement in graphicsCommands:
                ...
                if command == "GoTo":
                    ...
                    intertext = attr["intertext"].value
                    fontsize = int(attr["fontsize"].value)
                    cmd = TextCommand(intertext, fontsize)
        ...
        textLabel = tkinter.Label(sideBar,text="Text Size")
        textLabel.pack()
        fontsize = tkinter.IntVar()
        textEntry = tkinter.Entry(sideBar,textvariable=fontsize)
        textEntry.pack()

        messageLabel = tkinter.Label(sideBar, text="Message")
        messageLabel.pack()
        message = tkinter.StringVar()
        messageEntry = tkinter.Entry(sideBar, textvariable=message)
        messageEntry.pack()

        def textMessageHandler():
            cmd = TextCommand(message.get(), fontsize.get())
            cmd.draw(theTurtle)
            self.graphicsCommands.append(cmd)
            screen.update()
            screen.listen()

        textButton = tkinter.Button(sideBar, text="Print Message", command=textMessageHandler)
